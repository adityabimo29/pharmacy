...
if (empty($_SESSION['namaadmin']) AND empty($_SESSION['leveladmin'])){
	...
	elseif ($module==$module2 AND $act=='downloadDistributor'){
		$row = $pdo->query("SELECT * FROM distributor")->fetchAll();
		$i= 2;
		$no=1;
		...
		foreach($row as $data){
			...
			$sheet->setCellValue('C'.$i, $data['kode_distributor']);
			$sheet->setCellValue('D'.$i, $data['alamat']);
			$sheet->setCellValue('E'.$i, $data['npwp']);
			$sheet->setCellValue('F'.$i, $data['no_hp']);
			$i++;
			$no++;
		}
		$writer = new Xlsx($spreadsheet);
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="Data Backup Distributor.xlsx"');
		$writer->save("php://output");
		// $writer->save('Template Excel Distributor.xlsx');
		
		//echo "<script>window.location = '../../backup'</script>";
	}

	elseif ($module==$module2 AND $act=='downloadObat'){
		$row = $pdo->query("SELECT * FROM obat")->fetchAll();
		$i= 2;
		$no=1;
		$spreadsheet = new Spreadsheet();
		$sheet = $spreadsheet->getActiveSheet();
		...
		foreach($row as $data){
			...
			$sheet->setCellValue('D'.$i, $data['kode_distributor']);
			$sheet->setCellValue('E'.$i, $data['stok']);
			$i++;
			$no++;
		}
		$writer = new Xlsx($spreadsheet);
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="Data Backup Obat.xlsx"');
		$writer->save("php://output");
		// $writer->save('Template Excel Distributor.xlsx');
