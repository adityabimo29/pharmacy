...
if (empty($_SESSION['namaadmin']) AND empty($_SESSION['leveladmin'])){
	...
	if ($module==$module2 AND $act=='update'){
		...
		if (empty($nama_file)){
			try {
				...
				$statement->bindParam(":id_jurnal", $_POST["id_jurnal"], PDO::PARAM_INT);
				$count = $statement->execute();
			
				echo "<script>window.location = '../../$module-edit-$_POST[id_jurnal]'</script>";
			}catch(PDOException $e){
				echo "<script>alert('$hal Gagal diedit!'); window.location = '../../$module-edit-$_POST[id_jurnal]'</script>";
			}
		}else{
			$edit = $pdo->query("SELECT gambar FROM jurnal WHERE id_jurnal='$_POST[id_jurnal]'");
			...
			try {
				...
				UploadAll($nama_file_unik,'jurnal',1200,0); //nama_foto,folder,lebar,tinggi
	...
	elseif ($module==$module2 AND $act=='add'){
		...
		if(empty($nama_file)){
			...
			try{
				...
				$stmt = $pdo->prepare("INSERT INTO jurnal
	...
	elseif ($module==$module2 AND $act=='remove'){
		$edit = $pdo->query("SELECT gambar FROM jurnal WHERE id_jurnal='$_GET[id]'");
		$rr = $edit->fetch(PDO::FETCH_ASSOC);
		unlink("../../../images/$module2/$rr[gambar]");
		unlink("../../../images/$module2/small/$rr[gambar]");
		
		$del = $pdo->query("DELETE FROM jurnal WHERE id_jurnal='$_GET[id]'");
		$del->execute();
	...
	elseif ($module==$module2 AND $act=='removeimg'){
		$edit = $pdo->query("SELECT id_jurnal,gambar FROM jurnal WHERE id_jurnal='$_GET[id]'");
		$tedit = $edit->fetch(PDO::FETCH_ASSOC);
		unlink("../../../images/$module2/$tedit[gambar]");
		unlink("../../../images/$module2/small/$tedit[gambar]");
		
		$statement = $pdo->prepare("UPDATE jurnal SET gambar='' WHERE id_jurnal='$_GET[id]'");
		$count = $statement->execute();
		
		header('location:../../'.$module2.'-edit-'.$tedit['id_jurnal']);
	}
