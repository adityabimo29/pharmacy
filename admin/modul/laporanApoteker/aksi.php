	...
	elseif ($module==$module2 AND $act=='printLaporan'){
		...
		try{
			...
			echo "<style>
				...
				<hr>
				<h3 class='bct' style='text-align:center'>LAPORAN HARIAN PENJUALAN</h3>
				<hr>
				<p>Nama   : ".$_SESSION['namaadmin']."</p>
				<p>Tanggal : ".$tgl."</p>
				<hr>
				<table>
					<tr>
					...
					$tgl2 = date('Y-m-d');
					$data = $pdo->query("SELECT * FROM penjualan WHERE id_admin = '$admin' AND tanggal = '$tgl2'  ")->fetchAll();
					foreach($data as $row){
					echo "
						<tr>
							<td>".$no."</td>
			...
			ob_end_clean();
			$mpdf->WriteHTML($html);
			$mpdf->Output('Laporan_Harian.pdf',\Mpdf\Output\Destination::DOWNLOAD);
