	...
	elseif ($module==$module2 AND $act=='printLaporan'){
		...
		try{
			...
			if(isset($_POST['nama_distributor']) && $_POST['nama_distributor'] !== ''){
				...
				$data = $pdo->query("SELECT * FROM pembelian WHERE ( tanggal BETWEEN '$date' AND '$date2' )   ")->fetchAll();
			}
			...
			echo "<style>
				...
				<hr>
				<h3 class='bct' style='text-align:center'>LAPORAN BARANG MASUK</h3>
				<hr>
				<p>Distributor   : ".$nama."</p>
			...
			ob_end_clean();
			$mpdf->WriteHTML($html);
			$mpdf->Output('Laporan_Barang_Masuk.pdf',\Mpdf\Output\Destination::DOWNLOAD);
