	...
	elseif ($module==$module2 AND $act=='printLaporan'){
		...
		try{
			...
			echo "<style>
				...
				<hr>
				<h3 class='bct' style='text-align:center'>LAPORAN STOK HARIAN</h3>
				<hr>
				...
				<table>
					...
					$tgl2 = date('Y-m-d');
					$data = $pdo->query("SELECT * FROM pembelian WHERE id_admin = '$admin' AND tanggal = '$tgl2'  ")->fetchAll();
					foreach($data as $row){
					echo "
						<tr>
							<td>".$no."</td>
			...
			ob_end_clean();
			$mpdf->WriteHTML($html);
			$mpdf->Output('Laporan_Stok_Harian.pdf',\Mpdf\Output\Destination::DOWNLOAD);
